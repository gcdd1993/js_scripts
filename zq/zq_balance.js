...
$.msg = ""
const re = /zqkey=(.*)&zqkey_id=(.*)&uid=(\d+)/

!(async () => {
    const zqKeyIds = process.env.ZQ_KEYID.split("@")
    for (let i = 0; i < zqKeyIds.length; i++) {
        $.index = i
        let match = zqKeyIds[i].match(re)
        $.zqKey = match[1]
        $.zqKeyId = match[2]
        $.uid = match[3]
...
async function getBalance() {
    const options = {
        'method': 'GET',
        'url': `https://kd.youth.cn/wap/user/balance?access=WIFI&app-version=3.6.0&app_version=3.6.0&cookie=${$.zqKey}&cookie_id=${$.zqKeyId}&zqkey=${$.zqKey}&zqkey_id=${$.zqKeyId}&uid=${$.uid}`,
        'headers': {
            'Host': 'kd.youth.cn',
